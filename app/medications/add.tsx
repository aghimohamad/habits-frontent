...
export default function AddMedicationScreen() {
  ...
  const handleFrequencySelect = (freq: string) => {
    ...
    if (selectedFreq) {
      setForm((prev) => ({
        ...
        // Create a new array with default times based on frequency
        times: [...selectedFreq.times], // Use spread operator to create a new array
      }));
    }
    if (errors.frequency) {
  ...
  const handleDurationSelect = (dur: number) => {
    ...
    const selectedDur = DURATIONS.find((d) => d.value === dur); // Update to find by value
    setForm((prev) => ({ ...prev, duration: selectedDur ? selectedDur.label : "" })); // Set duration label
    if (errors.duration) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // await scheduleMedicationReminder(form);
      // if (form.refillReminder) {
      //   await scheduleRefillReminder(form);
      // }
      ...
      <View style={styles.content}>
        ...
        >
                ...
                >
                  ...
                  >
                    <Ionicons
                      ...
                      size={24}
                      color={
                        selectedFrequency === freq.id ? "white" : "#1a8e2d"
                      }
                    />
                  </View>
                  <Text
            ...
            <View style={styles.optionsGrid}>
              {DURATIONS.map((duration) => (
                ...
                >
                  ...
                  >
                    {duration.value === -1 ? "Ongoing" : duration.value}
                  </Text>
                  <Text
                    style={[
                      styles.optionLabel,
          ...
          {showTimePicker && (
            <DateTimePicker
              value={(() => {
                const currentTime = form.times[editingTimeIndex || 0];
